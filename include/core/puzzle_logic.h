...
// Returns true when both specs describe the same grid size and theme.
...
// Returns true when the spec is one of the two supported puzzle configurations
// (3x3 Planets or 4x3 Greek).
...
// Extracts the PuzzleSpec (dimensions and theme) from a full PuzzleState.
inline PuzzleSpec puzzleSpec(const PuzzleState& state) {
    return {state.columns, state.rows, state.theme};
}

// Returns the total number of tiles (columns * rows), including the blank.
inline uint8_t puzzleTileCount(const PuzzleState& state) {
    return static_cast<uint8_t>(state.columns * state.rows);
}

// Creates a solved puzzle in the goal arrangement. Tiles are numbered 1..(n-1)
// left-to-right, top-to-bottom; the blank occupies the last position.
...
// Returns true when the two tile positions are horizontally or vertically
...
// Creates a pseudo-randomly scrambled puzzle derived from gameId and the board
// identities. Guarantees the result is not already solved.
...
// Returns true when the tile arrangement can be reached from the solved state
// via legal moves (inversion-parity rule).
...
// Returns true when the state has a supported spec, valid tile permutation,
// correct phase, non-zero IDs, and a solvable arrangement.
...
// Returns the index of the blank tile (value 0), or puzzleTileCount() if not
// found (should never happen in a valid state).
...
// Returns true when the tile at the given position is already in its goal slot.
...
// Returns true when every numbered tile occupies its goal position and the
// blank is in the last slot.
...
// Returns true when both states represent the exact same puzzle (all fields
// including tiles, revision, and turn ownership).
...
// Compares two puzzle states by epoch (gameId) then revision to determine
...
// Computes a FNV-1a digest over all state fields for integrity checks and
...
// Returns true when the observed state has superseded a pending delivery,
// indicating the delivery no longer needs to be sent.
