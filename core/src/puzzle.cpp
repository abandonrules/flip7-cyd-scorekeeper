/**
 * Flip7 Core - Puzzle Game Logic
 * Platform-neutral sliding puzzle logic
 */
...
PuzzleSpec makePuzzleSpec(uint8_t columns, uint8_t rows, PuzzleTheme theme) {
    return {columns, rows, theme};
}

bool isSupportedPuzzleSpec(const PuzzleSpec& spec) {
    return (spec.columns == 3 && spec.rows == 3 && spec.theme == PuzzleTheme::Planets) ||
           (spec.columns == 4 && spec.rows == 3 && spec.theme == PuzzleTheme::Greek);
}
...
inline bool arePositionsAdjacent(const PuzzleState& state, uint8_t first, uint8_t second) {
    ...
    const uint8_t firstCol = first % state.columns;
    const uint8_t secondRow = second / state.columns;
    const uint8_t secondCol = second % state.columns;

    const uint8_t rowDist = firstRow > secondRow ? firstRow - secondRow : secondRow - firstRow;
    const uint8_t colDist = firstCol > secondCol ? firstCol - secondCol : secondCol - firstCol;
...
inline PuzzleState makeScrambledPuzzle(uint32_t firstTurnBoardId, uint32_t gameId, const PuzzleSpec& spec) {
    ...
    for (uint16_t step = 0; step < steps; ++step) {
        ...
        for (uint8_t pos = 0; pos < count; ++pos) {
            if (pos != previousBlank && arePositionsAdjacent(state, pos, blank)) {
    ...
    for (uint8_t pos = 0; solved && pos + 1 < count; ++pos) {
        solved = state.tiles[pos] == pos + 1;
    }
...
inline bool isSolvablePuzzleArrangement(const PuzzleState& state) {
    ...
    for (uint8_t first = 0; first < count; ++first) {
        ...
        for (uint8_t second = first + 1; second < count; ++second) {
            if (state.tiles[second] != 0 && state.tiles[first] > state.tiles[second]) {
                ++inversions;
            }
        }
    }
    ...
    uint8_t blank = count;
    for (uint8_t pos = 0; pos < count; ++pos) {
        if (state.tiles[pos] == 0) {
            blank = pos;
            break;
        }
    }

    if (blank == count) return false;

    const uint8_t blankRowFromBottom = state.rows - blank / state.columns;
...
inline bool isValidPuzzle(const PuzzleState& state) {
    ...
    for (uint8_t pos = 0; pos < count; ++pos) {
        const uint8_t tile = state.tiles[pos];
        if (tile >= count || seen[tile]) return false;
        seen[tile] = true;
    }
    for (uint8_t pos = count; pos < kPuzzleMaxTiles; ++pos) {
        if (state.tiles[pos] != 0) return false;
    }
...
inline bool isPuzzleSolved(const PuzzleState& state) {
    ...
    for (uint8_t pos = 0; pos + 1 < count; ++pos) {
        if (!isTileCorrect(state, pos)) return false;
    }
...
inline bool isSamePuzzle(const PuzzleState& first, const PuzzleState& second) {
    ...
    for (uint8_t i = 0; i < kPuzzleMaxTiles; ++i) {
        if (first.tiles[i] != second.tiles[i]) return false;
    }
    return true;
}
...
inline PuzzleVersionOrder comparePuzzleVersion(const PuzzleState& current, const PuzzleState& candidate) {
    ...
    if (candidate.revision != current.revision) {
        return candidate.revision > current.revision ? PuzzleVersionOrder::Newer : PuzzleVersionOrder::Older;
    }
...
inline uint32_t puzzleStateDigest(const PuzzleState& state) {
    ...
    for (uint8_t tile : state.tiles) addByte(tile);
    addByte(state.columns);
    addByte(state.rows);
    addByte(static_cast<uint8_t>(state.theme));
    addByte(static_cast<uint8_t>(state.phase));
    const uint32_t fields[] = {state.gameId, state.revision, state.turnBoardId};
...
inline bool tryPuzzleMove(PuzzleState& state, uint8_t position,
    ...
    if (!arePositionsAdjacent(state, position, blank)) return false;
...
inline bool exitPuzzle(PuzzleState& state, uint32_t actorBoardId, uint32_t otherBoardId) {
    if (!isValidPuzzle(state) || state.phase != PuzzlePhase::Playing ||
        ...
        (state.turnBoardId != actorBoardId && state.turnBoardId != otherBoardId)) {
        return false;
    }
...
inline bool applyPuzzleExitSignal(PuzzleState& current, const PuzzleState& candidate,
    ...
    if (!isValidPuzzle(current) || !isValidPuzzle(candidate) ||
        ...
        current.columns != candidate.columns || current.rows != candidate.rows ||
        current.theme != candidate.theme ||
        senderBoardId == 0 || recipientBoardId == 0 ||
        senderBoardId == recipientBoardId ||
        (current.turnBoardId != senderBoardId && current.turnBoardId != recipientBoardId)) {
        return false;
    }
    if (candidate.turnBoardId != senderBoardId && candidate.turnBoardId != recipientBoardId) {
        return false;
    }

    const PuzzleVersionOrder order = comparePuzzleVersion(current, candidate);
    const bool equalConflict = order == PuzzleVersionOrder::Same && !isSamePuzzle(current, candidate);
...
inline bool isValidRemoteTransition(const PuzzleState& current, const PuzzleState& incoming,
    ...
    if (!isValidPuzzle(current) || !isValidPuzzle(incoming) ||
        ...
        current.columns != incoming.columns || current.rows != incoming.rows ||
    ...
    if (!arePositionsAdjacent(current, oldBlank, newBlank) ||
        incoming.tiles[oldBlank] != current.tiles[newBlank]) {
        return false;
    }

    for (uint8_t i = 0; i < kPuzzleMaxTiles; ++i) {
        if (i != oldBlank && i != newBlank && incoming.tiles[i] != current.tiles[i]) {
            return false;
        }
    }
    return true;
...
inline bool isValidPuzzleForParticipants(const PuzzleState& state,
    ...
    if (state.turnBoardId != firstBoardId && state.turnBoardId != secondBoardId) return false;
    if (state.phase == PuzzlePhase::Exited) return state.revision != 0;

    const uint32_t host = firstBoardId < secondBoardId ? firstBoardId : secondBoardId;
    const uint32_t guest = firstBoardId < secondBoardId ? secondBoardId : firstBoardId;
...
inline bool shouldAdoptFullState(const PuzzleState& current, const PuzzleState& incoming,
    ...
    const uint32_t host = senderBoardId < recipientBoardId ? senderBoardId : recipientBoardId;
    ...
    if (incoming.revision == current.revision + 1) {
        return isValidRemoteTransition(current, incoming, senderBoardId, recipientBoardId);
    }
